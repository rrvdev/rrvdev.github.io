...
(function () {
  ...
  const finalizarRachaBtn = document.getElementById('btn-finalizar-racha');
  ...
  const renderizarDashboard = () => {
    const artilheirosDoRacha = Object.entries(golsDoRacha).map(([id, gols]) => {
      const jogador = jogadores.find(j => j.id === id);
      return { ...jogador, golsRacha: gols };
    }).filter(j => j.golsRacha > 0).sort((a, b) => b.golsRacha - a.golsRacha);
    artilheirosContainer.innerHTML = artilheirosDoRacha.length === 0 ? '<li>Nenhum gol marcado neste racha.</li>' : '';
    artilheirosDoRacha.forEach((jog, index) => {
      const li = document.createElement('li');
      li.innerHTML = `<span class="rank rank-${index + 1}">${index + 1}</span>
                      <span class="player-name">${jog.nome}</span>
                      <span class="player-stat">${jog.golsRacha || 0} Gols</span>`;
  ...
  const renderizarControleGols = () => {
    ...
    const jogadoresPresentes = jogadores.filter(j => j.presente).sort((a, b) => a.nome.localeCompare(b.nome));
    ...
    jogadoresPresentes.forEach(jog => {
      ...
      card.querySelector('span').textContent = golsDoRacha[jog.id] || 0;
      card.querySelectorAll('.btn-gol').forEach(btn => btn.disabled = !rachaEmAndamento);
  ...
  const renderizarAvaliacoes = () => {
    if (votacaoAberta) {
        ...
        jogadoresPresentes.forEach(j => {
            const option = document.createElement('option');
            option.value = j.id;
            option.textContent = j.nome;
            if (votosDoRacha[j.id]) {
        ...
        voterSelect.dispatchEvent(new Event('change')); // Força a atualização da regra de auto-voto
    } else {
  ...
  const renderizarTimes = () => {
    ...
    times.forEach((time, idx) => {
      ...
      time.sort((a,b) => a.nome.localeCompare(b.nome)).forEach(jog => {
    ...
    const jogadoresNosTimesIds = new Set(times.flat().map(j => j.id));
    const jogadoresSemTime = jogadores.filter(j => j.presente && !jogadoresNosTimesIds.has(j.id));
    if (jogadoresSemTime.length > 0) {
      reservesContainer.style.display = 'block';
      jogadoresSemTime.forEach(jog => {
        const isGoleiro = jog.setor.toUpperCase() === 'GOLEIRO';
        const item = document.createElement('div');
        item.className = 'player-item-draggable';
        item.dataset.playerId = jog.id;
        item.innerHTML = `
            ${isGoleiro ? '<i class="fas fa-mitten"></i> ' : ''}
            ${jog.nome} 
            <span class="player-item-stars">(${(jog.estrelas || 1).toFixed(1)})</span>
  ...
  // --- LÓGICA DE JOGO, SORTEIO E CONTROLE ---
  const handleTogglePresenca = async (playerId) => {
    const jogador = jogadores.find(j => j.id === playerId);
    if (!jogador) return;
    const novaPresenca = !jogador.presente;
    const { error } = await supabaseClient.from('jogadores').update({ presente: novaPresenca }).eq('id', playerId);
    if (error) {
      showToast(`Erro ao atualizar presença de ${jogador.nome}`);
    } else {
      showToast(`Presença de ${jogador.nome} atualizada.`);
    }
  };
  const handleCadastro = async (e) => {
    ...
    if (error) return showToast('Erro ao adicionar jogador.');
  ...
  const handleDeletarJogador = async (id) => {
    ...
    if (error) return showToast('Erro ao excluir jogador.');
  ...
  const handleSortearTimes = (e) => {
    ...
    if (jogadoresPresentes.length < numTimes) {
      return showToast('Não há jogadores presentes suficientes para formar os times.');
    }
    const goleiros = jogadoresPresentes.filter(j => j.setor.toUpperCase() === 'GOLEIRO').sort((a, b) => (b.estrelas || 1) - (a.estrelas || 1));
    const jogadoresDeLinha = jogadoresPresentes.filter(j => j.setor.toUpperCase() !== 'GOLEIRO').sort((a, b) => (b.estrelas || 1) - (a.estrelas || 1));
    times = Array.from({ length: numTimes }, () => []);
    const todosParaDistribuir = [...goleiros, ...jogadoresDeLinha];
    let timeIndex = 0;
    while (todosParaDistribuir.length > 0) {
      times[timeIndex % numTimes].push(todosParaDistribuir.shift());
      timeIndex++;
    }
    renderizarTimes();
  ...
  const handleSalvarTimesEditados = () => {
    const novosTimes = [];
    document.querySelectorAll('.team .team-player-list').forEach(teamEl => {
      const time = [];
      teamEl.querySelectorAll('.player-item-draggable').forEach(playerEl => {
        const jogador = jogadores.find(j => j.id === playerEl.dataset.playerId);
        if (jogador) time.push(jogador);
      });
    ...
    renderizarTimes();
    mudarTela('jogo');
    showToast("Times salvos! Racha pronto para começar.");
  };
  const habilitarEdicaoTimes = (ativar) => {
    ...
    const allLists = document.querySelectorAll('.team-player-list');
    if (ativar) {
  ...
  const finalizarRacha = async () => {
    if (!rachaEmAndamento && tempoRestanteSegundos === 0 && Object.keys(golsDoRacha).length === 0) return showToast("O racha não foi iniciado.");
    clearInterval(cronometroInterval);
    cronometroInterval = null;
    rachaEmAndamento = false;
    startPauseBtn.textContent = 'Iniciar Racha';
    showToast("Racha Finalizado! Salvando dados e liberando votação...");
    renderizarControleGols();

    const updates = Object.keys(golsDoRacha).map(jogadorId => {
      if (golsDoRacha[jogadorId] > 0) {
        const jogadorOriginal = jogadores.find(j => j.id === jogadorId);
        if (jogadorOriginal) {
          const novosGols = (jogadorOriginal.gols || 0) + golsDoRacha[jogadorId];
          const novasPartidas = (jogadorOriginal.partidas || 0) + 1;
          return supabaseClient.from('jogadores').update({ gols: novosGols, partidas: novasPartidas }).eq('id', jogadorId);
        }
      }
      return null;
    });
    await Promise.all(updates.filter(p => p));
    showToast("Gols salvos com sucesso!");
  ...
  const handleStartPause = () => {
    if (rachaEmAndamento) {
      ...
      showToast("Racha pausado.");
    } else {
      if (times.length === 0) return showToast("Sorteie os times antes de iniciar.");
      if (tempoRestanteSegundos <= 0) {
        const duracaoMinutos = parseInt(gameDurationInput.value, 10);
        tempoRestanteSegundos = duracaoMinutos * 60;
      }
      rachaEmAndamento = true;
  ...
  const handleReset = (resetCompleto = true) => {
    ...
    startPauseBtn.textContent = 'Iniciar Racha';
    cronometroDisplay.classList.remove('finished');
    if (resetCompleto) showToast("Racha resetado.");
  };
  ...
  const handleImportPlayers = (e) => {
    ...
    reader.onload = (event) => parseAndAddPlayers(event.target.result);
  ...
  const parseAndAddPlayers = async (textContent) => {
    ...
    lines.forEach(line => {
      ...
      novosJogadores.push({ nome, setor, presente: true });
      importedCount++;
    });
    if (novosJogadores.length > 0) {
      const { error } = await supabaseClient.from('jogadores').insert(novosJogadores);
      if (error) return showToast("Erro ao importar jogadores.");
    }
  ...
  const handleEditPlayer = async (e) => {
    ...
    if (!novoNome || !novoSetor) return showToast("Nome e setor não podem estar vazios.");
    const { error } = await supabaseClient.from('jogadores').update({ nome: novoNome, setor: novoSetor }).eq('id', playerId);
    if (error) return showToast("Erro ao atualizar jogador.");
    closeEditModal();
    showToast("Jogador atualizado com sucesso!");
  };

  // --- FUNÇÕES DE DADOS E ATUALIZAÇÃO DE UI ---
  ...
  const init = async () => {
    ...
    finalizarRachaBtn.addEventListener('click', finalizarRacha);
    ...
    listaCompletaJogadoresContainer.addEventListener('click', (e) => {
      ...
      if (targetButton) {
        ...
        if (targetButton.classList.contains('btn-edit-player')) {
          ...
          const jogador = jogadores.find(j => j.id === playerId);
          openConfirmModal(`Tem certeza que deseja excluir ${jogador.nome}?`, () => handleDeletarJogador(playerId));
        }
        return;
      }
    ...
    controleGolsContainer.addEventListener('click', (e) => {
      if (!rachaEmAndamento) {
        showToast("O racha precisa ser iniciado para adicionar gols.");
        return;
      }
      if (e.target.classList.contains('btn-gol')) {
        ...
        golsDoRacha[id] = golsDoRacha[id] || 0;
        if (action === 'add') {
          golsDoRacha[id]++;
        } else if (action === 'remove' && golsDoRacha[id] > 0) {
          golsDoRacha[id]--;
        }
        card.querySelector('span').textContent = golsDoRacha[id];
      }
    });
    
    voterSelect.addEventListener('change', () => {
        const voterId = voterSelect.value;
        const allCards = avaliacaoListaContainer.querySelectorAll('.card');
        allCards.forEach(card => {
            card.classList.toggle('is-disabled', card.dataset.id === voterId);
        });
    });
